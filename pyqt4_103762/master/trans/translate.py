...
class QGDW103762Class():
    ...
    def __init__(self):
        ...
        self.dll.GDW1376_2_parse_foy_py.argtypes = [
    ...
    def parse_all(self, instr):
        ...
        try:
            ...
            ret = self.dll.GDW1376_2_parse_foy_py(
                instr.encode('ascii', errors='ignore'),
                out, PARSE_OUT_BUF_SIZE)
...
class Translate:
    ...
    def get_full(self, is_show_level=True, is_show_type=True,
                 is_output_html=True, has_linklayer=True, indent_style='none'):
        """get full translate; indent_style ignored if format_parse_text unused path."""
        if self.is_success:
            res_text = ''
        else:
            if is_output_html:
                res_text = (
                    '<p style="color: red">报文解析过程出现问题，请检查报文。'
                    '若报文无问题请反馈660316，谢谢！</p><p> </p>')
            else:
                res_text = (
                    '报文解析过程出现问题，请检查报文。'
                    '若报文无问题请反馈660316，谢谢！\n\n')
    ...
    def get_clipboard_text(self, is_show_level=True, is_show_type=True,
        ...
        msg = self.source_msg
        full = self.get_full(
            is_show_level, is_show_type, is_output_html=False,
            indent_style=indent_style)
        brief = self.get_brief()

        if brief == '':
            text = '【报文】\n{msg}\n\n【完整解析】\n{full}'.format(
                msg=msg, brief=brief, full=full)
        else:
            ...
            text = (
                '【报文】\n{msg}\n\n【概览】\n{brief}\n\n【完整解析】\n{full}'
                .format(msg=msg, brief=brief, full=full))
        return text
